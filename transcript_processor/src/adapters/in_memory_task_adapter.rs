    ...
    for InMemoryTaskAdapter
{
    ...
    fn find_one(
        ...
        let found = match filter {
            crate::application::ports::task_repository_port::TaskFilter::ById(id) => {
                tasks.get(id).cloned()
            }
            crate::application::ports::task_repository_port::TaskFilter::ByStatus(status) => {
                tasks.values().find(|task| &task.status == status).cloned()
            }
            crate::application::ports::task_repository_port::TaskFilter::ByAssignee(assignee) => {
                tasks
                    .values()
                    .find(|task| {
                        task.assignee
            ...
            crate::application::ports::task_repository_port::TaskFilter::All => {
    ...
    fn find(
        ...
        if let std::option::Option::Some(sort_specs) = opts.sort {
            for sort_spec in sort_specs.iter().rev() {
                match &sort_spec.key {
                    crate::application::ports::task_repository_port::TaskSortKey::CreatedAt => {
                        filtered.sort_by_key(|task| task.created_at);
                    }
                    crate::application::ports::task_repository_port::TaskSortKey::UpdatedAt => {
                        filtered.sort_by_key(|task| task.updated_at);
                    }
                    crate::application::ports::task_repository_port::TaskSortKey::Status => {
                        filtered.sort_by(|a, b| {
                            std::format!("{:?}", a.status).cmp(&std::format!("{:?}", b.status))
                        });
                    }
                    crate::application::ports::task_repository_port::TaskSortKey::Title => {
                        filtered.sort_by(|a, b| a.title.cmp(&b.title));
                    }
                    crate::application::ports::task_repository_port::TaskSortKey::DueDate => {
                        filtered.sort_by(|a, b| {
...
impl crate::application::ports::task_repository_port::TaskRepositoryPort
    for InMemoryTaskAdapter
{
}
...
mod tests {
    ...
    fn test_save_and_find_one() {
        ...
        let find_result = adapter.find_one(
            &crate::application::ports::task_repository_port::TaskFilter::ById(std::string::String::from("task-1"))
        );
    ...
    fn test_find_by_status_filter() {
        ...
        let result = adapter.find(
            &crate::application::ports::task_repository_port::TaskFilter::ByStatus(
            ...
            hexser::ports::repository::FindOptions {
                sort: std::option::Option::Some(vec![
                    hexser::ports::repository::Sort {
                        key: crate::application::ports::task_repository_port::TaskSortKey::Title,
